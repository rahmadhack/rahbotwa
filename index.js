	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = ` [*SELAMAT DATANG @${num.split('@')[0]} DI GC ${mdata.subject}* ] *Member baru jangan sok kerass,Sok asik,gausah caperr,disuruh intro ya intro gk usah bnyak bacott*		\n╔╦╦╦═╦╗╔═╦═╦══╦═╗ \n║║║║═╣║║╔╣║║║║║═╣ \n║║║║═╣╚╣╚╣║║║║║═╣ \n╚══╩═╩═╩═╩═╩╩╩╩═╝                    \n*Baca dekripsi grub njg,ɪɴᴛʀᴏ/ᴅɪᴋɪᴄᴋ:*      \n✰❛New Member Intro✰❛   \n╔══════════════                   \n║❖➥ *Nama :*                        \n║❖➥ *Umur:*                         \n║❖➥ *Askot:*                         \n║❖➥ *Hobi:*                        \n║❖➥ *Status:*                         \n║ *SEMOGA BETAH GAIS,!*                     \n╚══════════════ `
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD Rizky Bot*',
					benned: 'Anda Ke Band Silahkan Hubungi Owner Agar Membuka Ban Anda',
					ownerG: '[❗] Lu spe owner? ini fitur khusus owner njierrr ❌',
					ownerB: '[❗] Lu spe owner? ini fitur khusus owner njierrr ❌',
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai user Rahmad Bot Daftar Untuk Berteman Dengan Rahmad Bot dengan cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar Rachmad|17\n\n──「 ঔৣ͜͡Rahmad Bot 」──`,
			...
			const ownerNumber = ["6288245621406@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6288245621406@s.whatsapp.net"]
			const adminbotnumber = ["6288245621406@s.whatsapp.net"]
			const frendsowner = ["6288245621406@s.whatsapp.net"]
			const premium = ["6288245621406@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ঔৣ͜͡Rahmad Bot ✨* 」\n*│+wa.me/628292030664*╰──────*「 *ঔৣ͜͡Rahmad Bot* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  Rahmad Bot_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281368646011*╰──────*「 * ঔৣ͜͡Rahmad Bot* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  ঔৣ͜͡Rahmad Bot BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
		...
		case 'iri':
            ...
            case 'bodoh':
				...
				if (!isPublic) return reply(mess.only.public)
					me = client.user
					uptime = process.uptime()
					teks = `*Nama bot* : ${me.name}\n*Author* : *ঔৣ͜͡Rahmad Bot*\n*Nomor Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Public:* ON\n*Total User Premium*: ${premium.length}\n*Total Chat* : ${totalchat.length}\n*Instagram* : *https:/instagram.com/rhmd_m*\n*Github* : *github.com/rhmadbot*\n*Youtube* : *-*`
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER ঔৣ͜͡Rahmad Bot👑* 」\n\`\`\``
					no = 0
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Gopay :* _0887-05000-212_\n*Saweria :* _https://saweria.co/rhmadbot_\n*Dana :* _0882-4562-1406_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * Rizky Bot Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner Rizky Bot!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin Rizky Bot!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
